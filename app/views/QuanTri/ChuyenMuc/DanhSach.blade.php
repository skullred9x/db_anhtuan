...
<section class="content">
	<div class="row">
		<div class="col-xs-12">
			<form action="{{ url('quantri/chuyen-muc/xoa') }}" method="post">
			<div class="box">
				<div class="box-header">
					<h3 class="box-title">Danh sách</h3>
					<div class="button-option"> <a href="{{ url('quantri/chuyen-muc/them-moi') }}" class="btn btn-success btn-flat" title="Thêm mới"><i class="fa  fa-plus-square-o"></i><span> Thêm mới</span> </a>
						<div class="button">
				...
				<div class="box-body table-responsive">
					<table id="example1" class="table table-bordered table-hover">
						...
						<tbody id="show">
							...
							<tr>
								<td><input type="checkbox" name="Chon[]" id="chk" value="{{ $item->dm_id }}" /></td>
								<td><a href="{{ url('quantri/chuyen-muc/sua-') }}{{ $item->dm_id }}" title="Sửa">{{ $item->dm_tieude }}</a></td>
								<td>{{ thuvien::PhanCapDM($item->dm_cha,true) }}</td>
								<td>{{{ $item->dm_trangthai==1 ? 'Hiện' : 'Ẩn' }}}</td>
								<td><a href="{{ url('quantri/chuyen-muc/sua-') }}{{ $item->dm_id }}" title="Sửa"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;<a href="{{ url('quantri/chuyen-muc/xoa-') }}{{ $item->dm_id }}" title="Xóa" onClick="return checkdelete();"><i class="fa fa-trash-o"></i></a></td>
							</tr>
							@endforeach
							@endif
						</tbody>
