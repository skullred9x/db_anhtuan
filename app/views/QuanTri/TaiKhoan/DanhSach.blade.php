...
<section class="content">
	<div class="row">
		<div class="col-xs-12">
			<form action="{{ url('quantri/tai-khoan/xoa') }}" method="post">
			<div class="box">
				<div class="box-header">
					<h3 class="box-title">Danh sách</h3>
					<div class="button-option"> <a href="{{ url('quantri/tai-khoan/them-moi') }}" class="btn btn-success btn-flat" title="Thêm mới"><i class="fa  fa-plus-square-o"></i><span> Thêm mới</span> </a>
						<div class="button">
				...
				<div class="box-body table-responsive">
					<table id="example1" class="table table-bordered table-hover">
						...
						<tbody id="show">
							...
							<tr>
								<td>{{ $item->tk_id }}</td>
								<td><input type="checkbox" name="Chon[]" id="chk" value="{{ $item->tk_id }}" /></td>
								<td>{{ $item->tk_tendangnhap }}</td>
								<td>{{ $item->tk_hoten }}</td>
								<td>{{ $item->tk_email }}</td>
								<td>{{ $item->tk_dienthoai }}</td>
								<td>{{{ $item->tk_trangthai==1 ? 'Kích hoạt' : 'Khóa' }}}</td>
								<td><a href="{{ url('quantri/tai-khoan/sua-') }}{{ $item->tk_id }}" title="Sửa"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;<a href="{{ url('quantri/tai-khoan/xoa-') }}{{ $item->tk_id }}" title="Xóa" onClick="return checkdelete();"><i class="fa fa-trash-o"></i></a></td>
							</tr>
							@endforeach
							@endif
						</tbody>
